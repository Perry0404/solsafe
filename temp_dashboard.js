...
export default function Dashboard() {
  ...
  const handleInitialize = async () => {
    ...
    try {
      ...
      console.log('≡ƒƒó Initializing program with validator integration...');
      ...
      console.log('Γ£à Transaction successful:', tx);
      alert(`Γ£à Program initialized successfully with validator integration!\n\nTransaction: ${tx}\n\nYou can now submit cases and validators can vote!`);
      await fetchCases();
    } catch (err) {
      console.error('≡ƒö┤ Initialize error:', err);
      if (err.message && err.message.includes('already in use')) {
        alert('Γ£à Program is already initialized!\n\nYou can now submit cases.');
        await fetchCases();
      } else {
        alert('Γ¥î Failed to initialize:\n\n' + (err.message || JSON.stringify(err)));
      }
    } finally {
            ...
            <div className="content">
              {!publicKey && (
                <div className="notice">
                  <h3>ΓÜá∩╕Å Wallet Not Connected</h3>
                ...
                <div>
                  ...
                  {publicKey ? (
                    <div>
                      ...
                      <p><strong>Program:</strong> {program ? 'Γ£à Connected' : 'Γ¥î Not loaded'}</p>
                    </div>
                  ) : (
